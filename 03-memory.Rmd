...
outnc <- nc_create("sst_wout_grid.nc", vars=sst_var, force_v4=T)
ncvar_put(outnc, sst_var, sst_base)
nc_close(outnc)
# set a proper grid description for cdo
system("cdo -setgrid,woa_1deg_griddes sst_wout_grid.nc sst.nc")
# note: woa_1deg_griddes is obtained from 'cdo griddes woa18_decav_t00_01.nc'
# https://www.nodc.noaa.gov/OC5/woa13/woa13data.html
cat(system("cdo griddes sst.nc | head -7 | tail -4", intern=T), sep="\n")
# save our arbitrary polygon
write.table(poly, file="poly.txt", row.names=F, col.names=F)
cat(system("cat poly.txt", intern=T), sep="\n")
# apply the polygon and calculate the cdo field mean
system("cdo -maskregion,poly.txt sst.nc sst_mask.nc")
# need to remove already existing file; dont know why
system("rm sst_mean_cdo.nc")
system("cdo -fldmean sst_mask.nc sst_mean_cdo.nc")
# load the result in R workspace
ncin <- nc_open("sst_mean_cdo.nc")
